...
void buffer_data_to_hash_buffer(u64 local_number_of_rows, u64* input_data, u64** outer_hash_data, u64* outer_hash_buffer_size, MPI_Comm comm)
{
...
int main(int argc, char **argv)
{
    ...
    for (u32 i1 = 0; i1 < relation_count; i1++)
    {
        ...
        if (read_offset + ceil((float)row_count[i1] / nprocs) > row_count[i1])
          local_row_count = row_count[i1] - read_offset;
        else
          local_row_count = (u64) ceil((float)row_count[i1] / nprocs);


        total_row_count = total_row_count + local_row_count;
    }

    u64 * input_buffer = new u64[total_row_count * COL_COUNT];

    //std::cout << "total_row_count: " << total_row_count << std::endl << std::endl;
    ...
    for (u32 i1 = 0; i1 < relation_count; i1++)
    {
        ...
        std::cout << "local_row_count: " << local_row_count << std::endl;
        std::cout << "bsize: " << bsize << std::endl;
        std::cout << "buffer_offset: " << buffer_offset << std::endl;
        std::cout << "read_offset: " << read_offset << std::endl << std::endl;
        */


        u64 rb_size = pread(fp, input_buffer + buffer_offset, bsize, read_offset * COL_COUNT * sizeof(u64));
        if (rb_size != bsize)
        {
            std::cout << "X Wrong IO: rank: " << rank << " " << rb_size << " " <<  bsize << " " << local_row_count << std::endl;
            MPI_Abort(MPI_COMM_WORLD, -1);
        }
        buffer_offset = buffer_offset + rb_size/sizeof(u64);
        close(fp);
    }
    ior_end = MPI_Wtime();

    hash_start = MPI_Wtime();
    buffer_data_to_hash_buffer(total_row_count, input_buffer, &(hashed_data), &(hash_entry_count), MPI_COMM_WORLD);
    hash_end = MPI_Wtime();

    union_start = MPI_Wtime();
    ...
    for ( u32 j1 = 0; j1 < hash_entry_count * COL_COUNT; j1 = j1 + 2)
    {
        total_tuple++;
        t1[0] = hashed_data[j1];
        t1[1] = hashed_data[j1+1];
        if (unionG.insert(t1))
            inserted_tuple++;
    }
    union_end = MPI_Wtime();
    ...
    if (max_time == elapsed_time)
    {
        ...
        {
            std::cout << "Row count: " << total_row_count
                      << " Hash row count: " << hash_entry_count
                      << " Read time: " << (ior_end - ior_start)
                      << " Hash time: " << (hash_end - hash_start)
                      << " Insert time: " << (union_end - union_start)
                      << std::endl;

            time = (ior_end - ior_start) + (hash_end - hash_start) + (union_end - union_start);
        }
