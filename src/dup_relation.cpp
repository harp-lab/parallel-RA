...
void testGoogleBtree(u64 * tup, u64 rc)
{
    ...
    */
    ...
    for(; i1 != map1->end(); i1++)
    {
        for (Relation0Map::iterator i0 = (i1->second)->begin(); i0 != (i1->second)->end(); i0++)
        {
            std::cout << "(" << i1->first << ", " << i0->first << ")" << std::endl;
            row_count++;
        }
    }
    //if (row_count != rc)
...
void check(u32 index, u64 rc, u64* read_buffer)
{
    ...
    std::cout << "[" << rc << "] Google " << google_elapsed.count() / 10 << std::endl;
