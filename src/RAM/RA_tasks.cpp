...
void RAM::execute_in_batches(int batch_size, std::vector<u32>& history, std::map<u64, u64>& intern_map, double *running_time, double *running_intra_bucket_comm, double *running_buffer_allocate, double *running_local_compute, double *running_all_to_all, double *running_buffer_free, double *running_insert_newt, double *running_insert_in_full, double *running_fp, std::vector<std::string>& debug_buffer, std::string debug_file_name)
{
    ...
    while (batch_size != 0)
    {
        ...
        if (mcomm.get_rank() == 0)
        {
            ...
            debug_buffer.push_back("--------------FIXED POINT ITERATION " + std::to_string(loop_count_tracker) + " --------------\n");
        }
#endif
        ...
        *running_time = *running_time + (insert_in_full_end - intra_start);
        ...
        if (mcomm.get_rank() == 0)
        {
            ...
            debug_buffer.push_back("Current time OUTER LOOP [" + std::to_string(loop_count_tracker) + " ] "
                      ...
                      + " Total " + std::to_string((insert_in_full_end - intra_start))
                      + " [ "
                      + std::to_string(*running_time)
                      + " ]\n");

            debug_buffer.push_back("Running time OUTER LOOP [" + std::to_string(loop_count_tracker) + "] "
    ...
    *running_fp = *running_fp + (fp_end - fp_start);
    if (mcomm.get_rank() == 0)
    {
        debug_buffer.push_back("Fixed Point [" + std::to_string(loop_count_tracker) + "] "
                  + std::to_string(fp_end - fp_start)
                  + " "
                  + std::to_string(*running_fp)
                  + "\n");
#if 0
