...
void google_relation::as_all_to_all_copy_buffer(int threshold, int RA_count, all_to_all_buffer& buffer, std::vector<u64> prefix, std::vector<int> reorder_map, int ra_id, u32 buckets, u32* output_sub_bucket_count, u32** output_sub_bucket_rank, u32 arity, u32 join_column_count, int head_rel_hash_col_count, bool canonical)
{
    google_relation *m_trie = this;
    for (u64 n : prefix)
    {
...
void google_relation::as_all_to_all_copy_buffer_helper(int threshold, int RA_count, google_relation*& cur_trie, std::vector<u64> cur_path, all_to_all_buffer& buffer, int ra_id, u32 buckets, u32* output_sub_bucket_count, u32** output_sub_bucket_rank, std::vector<int> reorder_map, u32 arity, u32 join_column_count, int head_rel_hash_col_count, bool canonical)
{
    if(cur_trie->is_end)
    {
        u64 reordered_cur_path[buffer.width[ra_id]];
...
void google_relation::as_all_to_all_copy_buffer_with_starting_index(int threshold, int RA_count, all_to_all_buffer& buffer, std::vector<u64> prefix, std::vector<int> reorder_map, int ra_id, u32 buckets, u32* output_sub_bucket_count, u32** output_sub_bucket_rank, u32 arity, u32 join_column_count, int head_rel_hash_col_count, bool canonical)
{
    ...
    for ( auto nxt = m_trie->next.begin(); nxt!= m_trie->next.end(); ++nxt )
    {
        std::vector<u64> prefix1;
        prefix1.push_back(nxt->first);
        u64 nxt_node = nxt->first;
        google_relation *temp_trie = m_trie->next[nxt_node];
...
void google_relation::as_all_to_all_copy_buffer_helper_with_starting_index(int threshold, int RA_count, google_relation*& cur_trie, std::vector<u64> cur_path, all_to_all_buffer& buffer, int ra_id, u32 buckets, u32* output_sub_bucket_count, u32** output_sub_bucket_rank, std::vector<int> reorder_map, u32 arity, u32 join_column_count, int head_rel_hash_col_count, bool canonical)
{
    if(cur_trie->is_end)
    {
        u64 reordered_cur_path[buffer.width[ra_id]];
...
void google_relation::as_all_to_all_copy_filter_buffer(int threshold, int RA_count, all_to_all_buffer& buffer, std::vector<u64> prefix, std::vector<int> reorder_map, int ra_id, u32 buckets, u32* output_sub_bucket_count, u32** output_sub_bucket_rank, u32 arity, u32 join_column_count, bool(*lambda)(const u64* const), int head_rel_hash_col_count, bool canonical)
{
    google_relation *m_trie = this;
    for (u64 n : prefix)
    {
...
void google_relation::as_all_to_all_copy_filter_buffer_helper(int threshold, int RA_count, google_relation*& cur_trie, std::vector<u64> cur_path, all_to_all_buffer& buffer, int ra_id, u32 buckets, u32* output_sub_bucket_count, u32** output_sub_bucket_rank, std::vector<int> reorder_map, u32 arity, u32 join_column_count, bool(*lambda)(const u64* const), int head_rel_hash_col_count, bool canonical)
{
    if(cur_trie->is_end)
    {
        u64 reordered_cur_path[buffer.width[ra_id]];
...
void google_relation::as_all_to_all_acopy_buffer(int threshold, int RA_count, all_to_all_buffer& buffer, std::vector<u64> prefix, std::vector<int> reorder_map, int ra_id, u32 buckets, u32* output_sub_bucket_count, u32** output_sub_bucket_rank, u32 arity, u32 join_column_count, int head_rel_hash_col_count, bool canonical)
{
    google_relation *m_trie = this;
    for (u64 n : prefix)
    {
...
void google_relation::as_all_to_all_acopy_buffer_helper(int threshold, int RA_count, google_relation*& cur_trie, std::vector<u64> cur_path, all_to_all_buffer& buffer, int ra_id, u32 buckets, u32* output_sub_bucket_count, u32** output_sub_bucket_rank, std::vector<int> reorder_map, u32 arity, u32 join_column_count, int head_rel_hash_col_count, bool canonical)
{
    if(cur_trie->is_end)
    {
        u64 reordered_cur_path[buffer.width[ra_id]];
...
void google_relation::as_all_to_all_right_join_buffer(int threshold, int RA_count, std::vector<u64> prefix, all_to_all_buffer& join_buffer, u64 *input0_buffer, int input0_buffer_width, int input1_buffer_width, int ra_id, u32 buckets, u32* output_sub_bucket_count, u32** output_sub_bucket_rank, std::vector<int> reorder_map, int join_column_count, google_relation& deduplicate, int *local_join_count, u32* local_join_duplicates, u32* local_join_inserts, std::string name, int head_rel_hash_col_count, bool canonical, u32* tracker)
{
...
void google_relation::as_all_to_all_right_join_buffer_helper(int threshold, int RA_count, google_relation*& cur_trie, std::vector<u64> cur_path, all_to_all_buffer& join_buffer, u64 *input0_buffer, int input0_buffer_width, int input1_buffer_width, int ra_id, u32 buckets, u32* output_sub_bucket_count, u32** output_sub_bucket_rank, std::vector<int> reorder_map, int join_column_count, google_relation& deduplicate, int *local_join_count, u32* local_join_duplicates, u32* local_join_inserts, std::string name, int head_rel_hash_col_count, bool canonical, u32* tracker)
{
    if(cur_trie->is_end)
    {
        u64 projected_path[join_buffer.width[ra_id]];
...
void google_relation::as_all_to_all_left_join_buffer(int threshold, int RA_count, std::vector<u64> prefix, all_to_all_buffer& join_buffer, u64 *input0_buffer, int input0_buffer_width, int input1_buffer_width, int ra_id, u32 buckets, u32* output_sub_bucket_count, u32** output_sub_bucket_rank, std::vector<int> reorder_map, int join_column_count, google_relation& deduplicate, int* local_join_count, u32* local_join_duplicates, u32* local_join_inserts, int head_rel_hash_col_count, bool canonical, u32* tracker)
{
    google_relation *m_trie = this;
    for (u64 n : prefix)  {
        if (m_trie->next.find(n)==m_trie->next.end())
...
void google_relation::as_all_to_all_left_join_buffer_helper(int threshold, int RA_count, google_relation*& cur_trie, std::vector<u64> cur_path, all_to_all_buffer& join_buffer, u64 *input0_buffer, int input0_buffer_width, int input1_buffer_width, int ra_id, u32 buckets, u32* output_sub_bucket_count, u32** output_sub_bucket_rank, std::vector<int> reorder_map, int join_column_count, google_relation& deduplicate, int* local_join_count, u32* local_join_duplicates, u32* local_join_inserts, int head_rel_hash_col_count, bool canonical, u32* tracker)
{
    if(cur_trie->is_end)
    {
        u64 projected_path[join_buffer.width[ra_id]];
