...
void google_relation::as_all_to_all_acopy_buffer(all_to_allv_buffer& buffer, std::vector<u64> prefix, std::vector<int> reorder_map, int ra_id, u32 buckets, u32* output_sub_bucket_count, u32** output_sub_bucket_rank, u32 arity, u32 join_column_count, int head_rel_hash_col_count, bool canonical)
{
    ...
    as_all_to_all_acopy_buffer_helper(m_trie, prefix, buffer, ra_id, buckets, output_sub_bucket_count, output_sub_bucket_rank, reorder_map, arity, join_column_count, head_rel_hash_col_count, canonical);
}



void google_relation::as_all_to_all_acopy_buffer_helper(google_relation*& cur_trie, std::vector<u64> cur_path, all_to_allv_buffer& buffer, int ra_id, u32 buckets, u32* output_sub_bucket_count, u32** output_sub_bucket_rank, std::vector<int> reorder_map, u32 arity, u32 join_column_count, int head_rel_hash_col_count, bool canonical)
{
    if(cur_trie->is_end)
    {
...
#if 0
        if (index == 0)
            std::cout << reordered_cur_path[0] << " "
                      << reordered_cur_path[1] << " "
                      << reordered_cur_path[2] << " "
                      << index << " "
                      << buffer.cumulative_tuple_process_map[index] << " "
                      << std::endl;
#endif
    }
    ...
    for (std::pair<u64, google_relation*> nxt: cur_trie->next)
    {
        ...
        as_all_to_all_acopy_buffer_helper(nxt_trie, cur_path, buffer, ra_id, buckets, output_sub_bucket_count, output_sub_bucket_rank, reorder_map, arity, join_column_count, head_rel_hash_col_count, canonical);
        cur_path.pop_back();
    }
}
...
void google_relation::as_all_to_all_copy_buffer(all_to_allv_buffer& buffer, std::vector<u64> prefix, std::vector<int> reorder_map, int ra_id, u32 buckets, u32* output_sub_bucket_count, u32** output_sub_bucket_rank, u32 arity, u32 join_column_count, int head_rel_hash_col_count, bool canonical)
{
    google_relation *m_trie = this;
    for (u64 n : prefix)
    {
    ...
    as_all_to_all_copy_buffer_helper(m_trie, prefix, buffer, ra_id, buckets, output_sub_bucket_count, output_sub_bucket_rank, reorder_map, arity, join_column_count, head_rel_hash_col_count, canonical);
}
...
void google_relation::as_all_to_all_copy_buffer_helper(google_relation*& cur_trie, std::vector<u64> cur_path, all_to_allv_buffer& buffer, int ra_id, u32 buckets, u32* output_sub_bucket_count, u32** output_sub_bucket_rank, std::vector<int> reorder_map, u32 arity, u32 join_column_count, int head_rel_hash_col_count, bool canonical)
{
    ...
    for (std::pair<u64, google_relation*> nxt: cur_trie->next)
    {
        ...
        google_relation *nxt_trie = nxt.second;
        cur_path.push_back(nxt_node);
        as_all_to_all_copy_buffer_helper(nxt_trie, cur_path, buffer, ra_id, buckets, output_sub_bucket_count, output_sub_bucket_rank, reorder_map, arity, join_column_count, head_rel_hash_col_count, canonical);
        cur_path.pop_back();
    }
}


void google_relation::as_all_to_all_copy_filter_buffer(all_to_allv_buffer& buffer, std::vector<u64> prefix, std::vector<int> reorder_map, int ra_id, u32 buckets, u32* output_sub_bucket_count, u32** output_sub_bucket_rank, u32 arity, u32 join_column_count, bool(*lambda)(const u64* const), int head_rel_hash_col_count, bool canonical)
{
    google_relation *m_trie = this;
    for (u64 n : prefix)
    {
    ...
    as_all_to_all_copy_filter_buffer_helper(m_trie, prefix, buffer, ra_id, buckets, output_sub_bucket_count, output_sub_bucket_rank, reorder_map, arity, join_column_count, lambda, head_rel_hash_col_count, canonical);
}



void google_relation::as_all_to_all_copy_filter_buffer_helper(google_relation*& cur_trie, std::vector<u64> cur_path, all_to_allv_buffer& buffer, int ra_id, u32 buckets, u32* output_sub_bucket_count, u32** output_sub_bucket_rank, std::vector<int> reorder_map, u32 arity, u32 join_column_count, bool(*lambda)(const u64* const), int head_rel_hash_col_count, bool canonical)
{
    if(cur_trie->is_end)
    {
        u64 reordered_cur_path[buffer.width[ra_id]];
    ...
    for (std::pair<u64, google_relation*> nxt: cur_trie->next)
    {
        ...
        google_relation *nxt_trie = nxt.second;
        cur_path.push_back(nxt_node);
        as_all_to_all_copy_filter_buffer_helper(nxt_trie, cur_path, buffer, ra_id, buckets, output_sub_bucket_count, output_sub_bucket_rank, reorder_map, arity, join_column_count, lambda, head_rel_hash_col_count, canonical);
        cur_path.pop_back();
    }
}
...
void google_relation::as_all_to_all_right_join_buffer_helper(google_relation*& cur_trie, std::vector<u64> cur_path, all_to_allv_buffer& join_buffer, u64 *input0_buffer, int input0_buffer_width, int input1_buffer_width, int ra_id, u32 buckets, u32* output_sub_bucket_count, u32** output_sub_bucket_rank, std::vector<int> reorder_map, int join_column_count, google_relation& deduplicate, int *local_join_count, u32* local_join_duplicates, u32* local_join_inserts, std::string name, int head_rel_hash_col_count, bool canonical)
{   
    if(cur_trie->is_end)
    {
#if 0
